...
<div>
	...
	<div class="relative mb-8 overflow-hidden rounded-2xl bg-slate-900 p-6 text-white md:p-8">
		...
		<div
			class="absolute right-1/4 bottom-1/4 h-36 w-36 rounded-full bg-emerald-500/10 blur-3xl"
		></div>
	...
	<div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 lg:grid-cols-4">
		...
		>
			...
			<p class="text-lg font-semibold text-slate-900 capitalize">{data.role}</p>
		</div>
		...
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
			...
			>
				...
				<p class="text-sm leading-relaxed text-slate-700">
					Chat with context-aware AI powered by RAG. Upload documents and get intelligent answers
					with citations.
				</p>
			</a>
			...
			{#if data.role === 'admin'}
				...
				>
					...
					<p class="text-sm leading-relaxed text-slate-700">
						Manage users, assign roles, view analytics, and monitor platform activity.
					</p>
				</a>
			{/if}
		</div>
	...
	<div class="overflow-hidden rounded-2xl bg-slate-900 px-6 py-4">
		<div class="flex flex-col items-center justify-between gap-3 text-sm sm:flex-row">
			<p class="text-slate-300">
				<span class="text-white">Auth</span><span class="text-lime-400">Flow</span> &mdash; Enterprise-grade
